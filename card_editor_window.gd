...
func _on_image_button_pressed() -> void:
	var command = load_txt_file("res://addons/card_multiplayer_json_editor/image_editor.txt").replace("%FILE%", ProjectSettings.globalize_path("res://Cards/svg/" + ACTIVE_CARD + ".svg"))
